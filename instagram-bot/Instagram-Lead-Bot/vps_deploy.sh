...
# ── مكتبة الصوت: اسمها تغيّر في Ubuntu 24.04 ──
UBUNTU_VER=$(. /etc/os-release && echo "$VERSION_ID")
if dpkg --compare-versions "$UBUNTU_VER" ge "24.04" 2>/dev/null; then
    LIBASOUND_PKG="libasound2t64"   # Ubuntu 24.04+
    info "Ubuntu 24.04 مكتشف — سيتم استخدام libasound2t64"
else
    LIBASOUND_PKG="libasound2"      # Ubuntu 22.04
    info "Ubuntu 22.04 مكتشف — سيتم استخدام libasound2"
fi

apt-get install -y --no-install-recommends \
    libnss3 libnss3-dev libnspr4 libatk1.0-0 libatk-bridge2.0-0 \
    libcups2 libdrm2 libxkbcommon0 libxcomposite1 libxdamage1 \
    libxfixes3 libxrandr2 libgbm1 "$LIBASOUND_PKG" \
